...
void loop() {
  ...
  if (lightValue > 900 && lightValue < 1100) {   //see if light is within range
    CircuitPlayground.strip.setPixelColor(0, 0, 255, 255);
    CircuitPlayground.strip.setPixelColor(1, 0, 255, 255);
  }

  if (soundValue > 300 && soundValue < 400) {    //see if sound is within range
    CircuitPlayground.strip.setPixelColor(6, 255, 255, 0);
    CircuitPlayground.strip.setPixelColor(7, 255, 255, 0);
  }

  if (tempValue > 500 && tempValue < 550) {     //see if temp is within range
    CircuitPlayground.strip.setPixelColor(3, 255, 0, 255);
    CircuitPlayground.strip.setPixelColor(4, 255, 0, 255);
  }
